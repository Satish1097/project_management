...
type FieldErrors = Partial<
  Record<'title' | 'projectId' | 'parentIssueId' | 'general', string>
>
...
export function CreateIssueDrawer({
  ...
  const { issues, createIssueViaApi } = useIssues()
  ...
  const parentIssueOptions = useMemo(() => {
    ...
    return issues
      .filter((issue) => issue.projectId === values.projectId && issue.issueType !== 'subtask')
      .map((issue) => ({
        value: issue.id,
        label: `${issue.key} — ${issue.title}`,
      }))
  }, [issues, values.projectId])
  ...
  const errors = useMemo((): FieldErrors => {
    ...
    if (values.issueType === 'subtask' && !values.parentIssueId) {
      next.parentIssueId = 'Parent issue is required for subtasks'
    }
    const parentError = apiErrors.parent_issue_id?.[0]
    if (parentError) next.parentIssueId = parentError
    const titleError = apiErrors.title?.[0]
  ...
  const isValid =
    ...
    (values.issueType !== 'subtask' || values.parentIssueId.length > 0) &&
    !errors.title &&
    !errors.projectId &&
    !errors.parentIssueId
  ...
  const submit = async (keepOpen: boolean) => {
    ...
    try {
      await createIssueViaApi(values.projectId, payload)
    } catch (error) {
  ...
  return (
    ...
    >
      ...
      >
        {errors.general && (
          <p className="rounded-lg border border-devflow-error/30 bg-devflow-error/5 px-3 py-2 text-body text-devflow-error">
            {errors.general}
          </p>
        )}
        ...
        <FormSection title="Classification">
          <IssueTypeSelect
            ...
            onChange={(issueType) => {
              update('issueType', issueType)
              if (issueType !== 'subtask') {
                update('parentIssueId', '')
              }
            }}
          />
          {values.issueType === 'subtask' && (
            <SelectField
              label="Parent issue"
              required
              value={values.parentIssueId}
              error={showError('parentIssueId')}
              options={[
                { value: '', label: 'Select parent issue…' },
                ...parentIssueOptions,
              ]}
              onChange={(e) => update('parentIssueId', e.target.value)}
              onBlur={() => setTouched((t) => ({ ...t, parentIssueId: true }))}
            />
          )}
