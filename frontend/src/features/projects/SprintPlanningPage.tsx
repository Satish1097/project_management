...
export function SprintPlanningPage() {
  ...
  const dropToSprint = (issueId: string) => {
    setMoveError(null)
    setDraggingId(null)
    void moveManyToSprintViaApi(projectId, sprintId, [issueId]).catch((error) => {
      setMoveError(
  ...
  const dropToBacklog = (issueId: string) => {
    setMoveError(null)
    setDraggingId(null)
    void moveManyToBacklogViaApi(projectId, sprintId, [issueId]).catch((error) => {
      setMoveError(
        error instanceof ApiError ? error.message : 'Failed to move issue to backlog.',
      )
    })
  }
