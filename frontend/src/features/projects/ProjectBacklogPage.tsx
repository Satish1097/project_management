...
export function ProjectBacklogPage() {
  ...
  const { issues, loadBacklog, backlogLoading, backlogError, moveManyToSprintViaApi } =
    useIssues()
  ...
  const addSelectedToSprint = () => {
    ...
    setMoveError(null)
    void moveManyToSprintViaApi(projectId, targetSprintId, [...selected])
      .then(() => setSelected(new Set()))
      .catch((error) => {
        setMoveError(
          error instanceof ApiError ? error.message : 'Failed to move issues to sprint.',
        )
      })
  }
  ...
  const dropToSprint = (sprintId: string) => {
    ...
    setMoveError(null)
    void moveManyToSprintViaApi(projectId, sprintId, [draggingId])
